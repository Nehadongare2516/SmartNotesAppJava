...
public class EditNote extends JPanel {
    ...
    public EditNote(JFrame host, long userId, Dashboard parent, long noteId) {
        this.host = host;
        this.userId = userId;
        this.parent = parent;
        this.noteId = noteId;
        ...
        JLabel header = new JLabel("Edit Note");
        ...
        RoundedButton saveBtn = new RoundedButton("Update", UiStyles.SUCCESS, Color.WHITE);
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (title.isEmpty() || content.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Title and content are required.", "Validation", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            try {
                new NoteDAO().updateNote(userId, noteId, catId, title, content);
                parent.refresh();
    ...
    private void loadNote() {
        try {
            ...
            if (note.getCategoryId() == null) {
                categoryCombo.setSelectedItem("Uncategorized");
            } else {
                // We only map category name by list (best-effort).
                for (int i = 0; i < categoryCombo.getItemCount(); i++) {
                    String name = categoryCombo.getItemAt(i);
                    Long id = new CategoryDAO().findCategoryIdByName(userId, name);
                    if (id != null && id.equals(note.getCategoryId())) {
